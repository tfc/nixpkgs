...
def expand_globs(paths: list[PathString]) -> list[PathString]:
    """expands all existing paths from globbed paths like bash would"""
...
def discover_reachable_paths(
    ...
    """Walks the file system tree and discovers all (via symlinks) reachable paths"""
...
def patterns_for_features(
    ...
    """
    Returns the list of patterns that are required for a given set of features
    >>> patterns = {
    ...   "a": {
    ...     "onFeatures": ["a"],
