...
}:
{
  ...
  config = lib.mkIf config.services.paretosecurity.enable {
    ...
    systemd.user.services.paretosecurity-user = lib.mkIf config.services.paretosecurity.trayIcon {
      wantedBy = [ "graphical-session.target" ];
    };
    systemd.user.timers.paretosecurity-user = lib.mkIf config.services.paretosecurity.trayIcon {
